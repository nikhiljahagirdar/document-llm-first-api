...
class SubcategoryDBService(BaseDBService):
    async def list_subcategories(self, conn: psycopg.AsyncConnection, limit: int = 100, offset: int = 0, search: Optional[str] = None) -> List[dict]:
        query = """
    ...
    async def get_all_subcategories_with_parents(self, conn: psycopg.AsyncConnection) -> List[dict]:
    ...
    async def get_subcategory(self, conn: psycopg.AsyncConnection, subcategory_id: uuid.UUID) -> Optional[dict]:
        query = "SELECT * FROM subcategories WHERE subcategory_id = %s::uuid"
        return await self.fetch_one(conn, query, (subcategory_id,))

    async def create_subcategory(self, conn: psycopg.AsyncConnection, category_id: uuid.UUID, name: str, description: Optional[str] = None, prompt: Optional[str] = None) -> dict:
        query = """
    ...
    async def update_subcategory(self, conn: psycopg.AsyncConnection, subcategory_id: uuid.UUID, update_data: dict) -> Optional[dict]:
        if not update_data:
    ...
    async def delete_subcategory(self, conn: psycopg.AsyncConnection, subcategory_id: uuid.UUID) -> bool:
